...
| `.venv/bin/python tizen-gbs-patch-suggest/scripts/run_patch_suggest.py --help` | pass | Launcher exposes `--evidence` and `--output-dir`; `--src-root` is intentionally PS-M2. |
| `.venv/bin/pytest tests/unit/test_patch_suggest.py -q` | pass | 6 PS-M1 tests passed. |
| `.venv/bin/pytest tests/ -q --cov=gbs_analyzer --cov-fail-under=95` | pass | 423 passed, coverage 95.97%. |
| `grep -rn "src_root\|src-root\|read_text" ... --exclude-dir=__pycache__` | pass | Runtime source reads are absent; only evidence JSON is read. |
