...
| d005 | Allow the outer Claude to write `.patch` suggestion files while still forbidding apply. | Real patch-suggest usage feedback after PS-M5. | Users need a file they can review and pass to `git apply`; writing a patch artifact is distinct from applying it. | `context.md` and `SKILL.md` now instruct Claude to save candidates as `candidate_N.patch`, recommend `git apply --check`, and still forbid `git apply` / `patch` execution. |
