...
7. Do not apply the patch. Present it to the user for review.

Result: `context.md` guides Claude to generate patch candidates without changing
the source tree.
...
6. Do not apply patches. Do not run `git apply` or `patch`. Do not modify the
   source tree. Present the patch draft to the user for review.
...
Any patch generated by Claude from `context.md` is a suggestion draft for human
review. The user decides whether to apply it.
