# Clang Fix Campaign v1.5.16 Final Design Review Package

## Package Identity

- Frozen source path:
  `docs/clang-fix-campaign/history/clang-fix-campaign-design-v1.5.16-FROZEN.md`
- Frozen SHA-256:
  `ff73f5e3c6d54a75ae60771b98eadcfc1a4d1422ca8faf337a3de09eee4346ff`
- Package generation commit: `b1ad87e`
- Contract status: v1.5.16-FROZEN

This file is self-contained. The complete frozen design appears after the
review request and version summary; repository access is not required.

## Review Request

Review the final text as a whole and return either `PASS` or findings. Prefix
every finding with one severity and one type:

- Severity: `[BLOCKER]`, `[MAJOR]`, `[MINOR]`, or `[NIT]`.
- Type: `[CODE_ISSUE]`, `[DESIGN_SUGGESTION]`, or `[ALTERNATIVE]`.

Pay special attention to:

1. Section 3.4: DDL, uniqueness, conditional payload constraints, and
   append-only semantics.
2. Section 4.1: frozen nine-step wrapper order and reconciliation branch/exit
   priority.
3. Section 4.2: frozen public APIs, transaction boundaries, and return values.

Finding closure follows R14: BLOCKER findings must be fixed; MAJOR findings
must be fixed or explicitly waived by the developer; MINOR findings enter the
development-memory TODO ledger; NIT findings are advisory.

## Version History Summary

| Version | Change | Main correction |
|---|---|---|
| v1.5.8 | 32 | invocation convergence uniqueness, receipt binding, CI evidence CHECK |
| v1.5.9 | 33 | crash recovery relinks before orphan backfill; HELD precheck reachability |
| v1.5.10 | 34 | atomic reconciliation API; recovery before evidence precheck |
| v1.5.11 | 35 | per-round grouping; deterministic PASS payload reconstruction |
| v1.5.12 | 36 | edit-spec based PASS-to-round attribution; unified grouping |
| v1.5.13 | 37 | ledger repair separated from current success; corruption fail-closed |
| v1.5.14 | 38 | round identity before success; A0 link/convergence integrity check |
| v1.5.15 | 39 | executable API/index checker rules and authority consolidation |
| v1.5.16 | 40 | checker uses `compile` to catch duplicate arguments; final freeze |

## Frozen Design Full Text

<!-- BEGIN FROZEN DESIGN: copied byte-for-byte from the source named above -->
# clang-fix-campaign 项目设计文档

## 0. 元信息

- 版本:**v1.5.16-FROZEN(实现输入版)**
- 创建时间:2026-07-29 最近修订:2026-08-04
- 状态:**Frozen**(2026-08-04,开发者确认;Frozen 后任何设计
  修改必须走 R1 并新建 `change_41.md` 或后续编号,禁止静默改写)
...
- 变更提案(`docs/clang-fix-campaign/design_changes/`):`change_1.md`(代码视角首审)、
  ...
  **撤销**、恢复 create_round → 对账原序(前移旁路了 round 身份
  ...
  裁决链头/SHA/后续编号同步)、旧条目:(**底层参数权威来源冻结**、校验先于
...
| `ORPHAN_PASS` | `round_index`, `arch_norm`, `verification_id`, `worktree_path`, `reason`(`link_failed`/`hash_mismatch`/`worktree_damaged`/**`ambiguous`**), `detected_at` | **去重**:同 `verification_id` 已有 ORPHAN_PASS 则不重复写(幂等);释放 API 按 `worktree_path` 枚举 |
...
| `CONVERGENCE` | `round_index`, `arch_norm`, **`invocation_event_id`(v1.5.8 补入契约——§4.1 早已要求 6a/6b 必带,但本表此前漏登,validator 按表校验会自拒合法事件):除 `reason='rebaselined'` 置 null 外**一律必填**;与真实列同名同值(v1.5.6 提升规则同 round_index/arch_norm),`append_event` 事务内校验其指向同 unit/round/arch 的 BUILD_INVOCATION(§4.2)**, **`result` 条件枚举(v1.5.8,B3——此前只写 `{PASS, FAIL}`,与 §4.1"result 对非 build 事件取 n_a"自相矛盾,orphan/rebaselined 合法事件会被 validator 自拒):`invocation_event_id` 非空且为 6a/6b 实质 build outcome → `PASS`/`FAIL`;`reason` ∈ {`orphan_invocation`, `rebaselined`, `apply_failed`, `analyzer_failed`, `toolchain_failed`, `previous_evidence_missing`} → `n_a`(此时 `verdict` 亦必为 `n_a`);其余组合 → PayloadSchemaError**, `verdict`(`advance`/`stalled`/`regressed`/`denied`/`n_a`), `reason`, **`evidence_path`/`evidence_sha256`:**仅"FAIL 且产出了 evidence"时必填**;PASS、apply_failed、analyzer 失败、工具链失败一律置 null(v1.5.3)**, **`verification_id`(PASS 时必填,供 6b 与 link 三方一致校验;FAIL 时 null)**, `actual_changed_paths[]`, `previous_basis`(`reproduce`/`prev_build`/`synthetic_zero`/**`none`**), `at` | 按 `(arch_norm)` 取 **event_id 最大**的一条 → 即"该 arch 最近一次 build 的结果";其 `evidence_path/sha256` 供下一轮作 previous |
...
| (终态释放) | **仅** REVIEW_PUSHED / REVIEW_INELIGIBLE(确定不再需要副本) | — | campaign 显式 `release_worktree_protection(worktree_path)` + WORKSPACE_RELEASE event;此后副本可被磁盘压力回收 |
...
| HELD_FOR_INVESTIGATION → REPAIR_ROUND_RUNNING(v1.5.4 恢复边) | `campaign-rebaseline` 成功:仅当 HELD 的 reason == `previous_evidence_missing`(其余 reason 一律拒),且新 REPRODUCE 与 `CONVERGENCE(verdict=n_a, reason=rebaselined)` **同事务**落库 | 新 REPRODUCE + rebaselined CONVERGENCE | 回到修复循环;**该 arch 的 SECONDARY_TARGET_ADOPTED 不重置**(豁免不因 rebaseline 复活) |
| HELD_FOR_INVESTIGATION(入口二,v1.4.6) | **单 arch** 的 orphan PASS:link 失败 / hash 不符 / 副本受损 / 多条歧义 | ORPHAN_PASS 事件(含 verification_id、worktree_path、reason)+ 该受保护副本 | **终态**;副本保留待查,经 `campaign-release-worktrees` 显式释放 |
...
python -m ci_triage campaign-repair-step   # v1.4.9 八步契约(取代 v1.4.3 的
  ...
  #   2) `create_round`(BEGIN IMMEDIATE;幂等三元身份;闸一 max_rounds;
  ...
  #          键**(真正有意义的自洽断言)
  #        · `realpath(--edit-spec)` 与其 sha256 必须等于该 round 记录
  ...
  #      branch='state_inconsistent_held'——
  ...
  #           不变量永久缺位。
  #      **出口优先级(v1.5.12 乙-M1 立,v1.5.13 丙-B4 修订)**:
  #      写入全部执行(clean 组 relink 与 d 补写照做),整体 branch 按
  ...
  #      随后 `consume_build_invocation`(BEGIN IMMEDIATE;闸二)→ 持有
  ...
  #      DB/config/派生规则唯一确定,**任何一项缺失即 exit 2,不得猜测**:
  ...
  #          该 arch 无 REPRODUCE 事件 → exit 2(状态机保证不会发生)
  ...
  **无人值守下 needs_confirmation = "带镣铐的 auto"**:允许继续,但受
  双闸预算 + convergence + 三 arch 门 + Gerrit 终审四重约束),继续走 convergence
  ...
  #          evidence_local;
  #        · 最新 CONVERGENCE 的 `result==PASS` → 用**内存生成的 synthetic
  ...
  #          按 reason 分流——
  #            `rebaselined` → **回退到最新 REPRODUCE 的 evidence_local**
  #              (rebaseline 的语义就是"以新基线重新开始");
  #            `orphan_invocation` / `apply_failed` / `analyzer_failed` /
  #            `toolchain_failed` → **沿 event_id 向前取最近一条
  #              verdict≠n_a 的 CONVERGENCE**;向前无实质事件 → 回退
  #              最新 REPRODUCE;
  ...
  **非空数组 schema 与顺序(v1.5.14 冻结)**:
  ...
  exit(v1.5.1 定案):
    **0 = 本次调用已正常完成并产出 verdict**(含 build FAIL 的正常路径;
    ...
    2 参数错;4 校验/预算/link 拒绝(REJECTED_IDENTITY_MISMATCH /
      RoundsExhausted / BudgetExhausted / link 失败);5 工具链或状态忙
...
def append_event(state_db, campaign_unit_key, event_type, payload) -> int:
    ...
    # 四值);**未在 §3.4 契约表中登记的 event_type 一律拒绝**
    ...
    # result/invocation_event_id 的条件枚举校验按 §3.4 契约表执行
...
def adopt_secondary_target_with_convergence(state_db, campaign_unit_key, *,
    ...
    # CK-API-01 对文档内全部冻结 API 代码块跑 ast.parse,物理防再发。
...
class ReconcileResult:        # v1.5.10(B1)新增;v1.5.11 扩展
    ...
    # exit 4;state_inconsistent_held → exit 4
...
def derive(worktree: Path, tree_sha, parent_sha, message,
    ...
    #     campaign 对**聚合成功组**在到达终态前绝不调用
    #     `release_worktree_protection`
    #     (终态 = REVIEW_PUSHED / REVIEW_MANUAL / REVIEW_INELIGIBLE /
    #     SANDBOX_QB_FAILED);终态后由 campaign 显式释放并记
...
def cleanup_disposable_copy(worktree_path: str,
    ...
    # 清理并记 WORKSPACE_CLEANUP event,再重跑同一 iter;**不得递增 iter
...
- **DoD 专项**:[ ] round CRUD 幂等与冲突用例(同 index 异 hash → StateInconsistent;
  ...
  **反向验证**:去掉索引(或改回 v1.5.7 谓词)后,②必须转绿——
  ...
  [ ] **n_a previous 分支**:最新事件为 `rebaselined` → 回退最新 REPRODUCE;
  为 `orphan_invocation`/`apply_failed` 等 → 向前取最近实质事件,无则
  REPRODUCE;**均不置 HELD**;`HELD → rebaseline → 重试`全链不得原地弹回;
...
| 12 | **全文 grep**:旧口径、占位符 `<...>`、null、版本号、**断链 `§n`**、**孤立错误码** | 每轮均有 |
...
| 7g | **每个 Python fence 可经 `compile(..., "exec")` 编译且 fence 外无模块式裸签名**(checker CK-API-01);**唯一权威 prompt 的索引 token 是 design 索引集合的非空子集**(CK-IDX-01);**§ 引用可解析**(CK-XREF-01);**Mermaid 节点先声明后引用**(CK-MMD-01)——四条均须配失败 fixture | 冻结签名重复形参(v1.5.7);`ux_convergence_once` 残留三版;"§2" 断链;Mermaid 前向引用 change_27 声称已修三版未落实 |

**机械动作(v1.5.0 补,清单不能只是表格)**:变更提案落盘前必须执行并附结果——
`grep -n "<[^>]*>" design.md | grep -v "^.*\`"`(占位符)、
`grep -on "§[0-9.]*" design.md | sort -u`(断链核对)、
`grep -c "REJECTED_[A-Z_]*" 各错误码是否同时出现在 §4.3`、
...
本文档为 **v1.5.16-FROZEN(实现输入版)**(2026-08-04;
冻结裁决见 §0)。
...
变更记录从 `change_41.md` 起,禁止直接覆盖本冻结契约。
