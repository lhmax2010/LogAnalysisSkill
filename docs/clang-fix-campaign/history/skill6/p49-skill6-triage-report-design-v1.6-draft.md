# P4.9 skill-6 设计:tizen-triage-report 抽取 + gbs_report 关门(v1.6-draft)
...
| 报表页登录态 | **`EXTERNAL_BRANCH`**:调用点 `fetch_gbs_report:70`,分支与 `raise` 位于 `tizen_ci_shared.quickbuild_http:190`;预期异常 `QuickBuildError`(登录页) | |
| iframe 缺失 | `NO_GBS_REPORT`(:74-78) | |
| iframe 页登录态 | **`EXTERNAL_BRANCH`**:调用点 `fetch_gbs_report:81`,同上;预期异常 `QuickBuildError` | |
...
`_raise_if_login_page` 的分支在 `tizen_ci_shared.quickbuild_http:190`,
本地 :70/:81 仅为调用点),则该行**不可能有诚实的本地 branch ID**。
...
**逐个计数**(本批无 elif);③**`assert`** 与 **`match`** 计入
`decision_points`(`match` 按其 `case` 数计)。
