# P4.9 skill-6 设计:tizen-triage-report 抽取 + gbs_report 关门(v1.0-draft)
...
**由脚本自源码 AST 机械生成,不手抄(⑩)**——实现期落表:gbs_report.py
...
——七约束前提成立。`EDIT_SPEC_SCHEMA` 的双权威在 `edit_spec_guard`(已随
skill-4)与 `gbs_patch_suggest/formatter`,**与 triage-report 无关**,skill-4
登记的关门批次为 patch-suggest,本批不接、不改其归宿。
...
| 6 | `find_iframe_src`/`_IframeParser`/`_attrs_to_map` 层位随完整闭包裁定;htmlutil 由真实复用面决定 | **执行**:三者**随 gbs_report 留 skill**;**不建 htmlutil** | 实测复用面:`_attrs_to_map` 两处独立定义(gbs_report:349 / qb-discover sources:279,返回类型不同),**无跨模块调用**——复用面为零,htmlutil 无存在依据 |
...
- **分支表(每行必有代码锚,实现期回写用例名)**:fetch 成功 / iframe 缺失
  ...
  **仅固化现状**;
- **⑧ arch**:`gbs_report.py` 含 `arch` 参数(:137 等)但**无分支逻辑**
  (透传给 URL 与结果),实测 grep 后按"有据豁免"或"透传维度一例"裁,
  实现期定;
