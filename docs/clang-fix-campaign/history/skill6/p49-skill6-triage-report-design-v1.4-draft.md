# P4.9 skill-6 设计:tizen-triage-report 抽取 + gbs_report 关门(v1.4-draft)
...
| `_status_from_anchor` text 回退(failed / succeeded) | 文本命中(:342/:344)(v1.4 补:**只用 class 型 anchor 的用例永不走此路径**) | |
...
| **arch 场景 A/B** | 见 §5 ⑧ 两场景 | |
**fixture 构造原则(v1.4 明写;两行覆盖 20 条件的前提)**:A 与 B 是**同一
数据结构的两个极端实例**——A 填满 `TriageReportData` 全部可选字段
(evidence/analyzer/primary_error/patch_context_*/warnings/errors/…),
B 全部置空/None。**两者合起来对每个 `if` 走真假两侧**;若实现期发现某条件
两侧无法由 A/B 覆盖(如互斥字段),**须单列一行并停止报告说明**,不得靠
"两行已过"蒙混。
...
- [ ] **三项同名件议题裁决关闭**(§3 skill-2 八组 + §3.1 **skill-3 D1** +
      **skill-4 D2**,非延期,论证按"维持 vs 改变"不对称性);
...
**准入检查(冻结前必过)**:分支表须**对每个被迁移模块声明其分支点总数**,
并与 **AST 枚举**对账——枚举面 = 全部 `raise` + 返回 `None`/多态返回 +
条件渲染(`if`/`else` 决定输出的位点);**模块级缺项即红**。
本批声明:`gbs_report.py` **13**(两家已逐一核对)、`report.py` **23**
(20 条件渲染 + `_primary_location` 三态)。
——如此"哪个模块没被扫到"会在冻结前自己暴露,而不靠评审方每轮换角度撞。

---
## 附:声明
零生产行为变更(两模块逐字节 + 一处 import 白名单);零判据变更;零新机制;
**本批实质 = 关门**:step-0 七约束 3 执行 4 未触发;**skill-2 八组、skill-3 D1、
skill-4 D2 三项议题统一关闭**;`EDIT_SPEC_SCHEMA` 归宿核实(归 patch-suggest)——**P4.9 抽取阶段的最后一笔欠债在此清零**。
