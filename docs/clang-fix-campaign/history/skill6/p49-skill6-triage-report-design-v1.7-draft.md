# P4.9 skill-6 设计:tizen-triage-report 抽取 + gbs_report 关门(v1.7-draft)
...
| 报表页登录态 | **`EXTERNAL_BRANCH`**:调用点 `fetch_gbs_report:70`,分支 `quickbuild_http:193:if` / raise `:198:raise`;预期异常 `QuickBuildError`,**code `COOKIE_EXPIRED`** | |
| iframe 缺失 | `NO_GBS_REPORT`(:74-78) | |
| iframe 页登录态 | **`EXTERNAL_BRANCH`**:调用点 `fetch_gbs_report:81`;分支 `quickbuild_http:193:if` / raise `:198:raise`;预期异常 `QuickBuildError`,code `COOKIE_EXPIRED` | |
...
| **畸形行跳过 ×4**(v1.3 找回:v1.1 有"畸形行",v1.2 重写时被删) | `_row_to_package` 四个 `return None`:cells<3(:313)/ **空 `spec_name`**(BoolOp 左操作数)/ 表头行 `package name`(BoolOp 右操作数)——**两个不同输入类各需一例**(:317,v1.7 补左操作数;对照 :339 的 `successful` 别名已补,同一构造不得一处补一处漏)/ 无 status anchor(:320)/ status 解析失败(:323) | |
| `render_report` **A:全部可选字段齐备** | 覆盖**普通 13 条的"真"侧**(:45/:47/:49/:64/:66/:89/:91/:93/:100/:102/:104/:108/:111)**+ :62/:77/:85/:81/:83 的"真"侧**;并使 :53/:70 走 `else`(即其"假"侧) | |
| `render_report` **B:全部可选字段缺失** | 覆盖**普通 13 条的"假"侧** **+ :53/:70 的"真"侧**(v1.6 更正:B **不**覆盖 :62/:77/:81/:83/:85——外层折叠后内层不可达) | |
| `render_report` **C:外层在、内层缺** | `selected_package` 存在但 `dest_file` 空;`source_fetch` 存在但 `change=None`、`error` 空 → 覆盖 **:62/:77/:85 的"假"侧** | |
| `render_report` **D:中层在、末层缺** | `source_fetch` 与 `change` 均存在,但 `number=None`、`matching_patchset=None` → 覆盖 **:81/:83 的"假"侧** | |
...
| `_looks_like_build_status_table` 三态 | status-anchor 命中 `:296:if`/`:297:return`;header fallback / 均不命中 `:299:return`(v1.6 更正::287 是 def 行、不在 ID 全集,§11 的 ⊆ 检查会红) | |
...
④**`ast.BoolOp`(v1.7 新增第四类,本批实存 14 处:gbs_report 13 + report 1)**
——BoolOp **本身不计入** `decision_points`(它不产生独立分支),但
**其各操作数若对应不同输入类,分支表须分行或在同行内列全**;
