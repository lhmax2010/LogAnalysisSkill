...
| 契约句 | 分支(代码锚) | 用例 |
|---|---|---|
| fetch 成功 | `fetch_gbs_report` 返回 `GbsReport`(:94);branch ID `fetch_gbs_report:94:return` | |
| 报表页登录态 | **`EXTERNAL_BRANCH`**:本地调用点 `fetch_gbs_report:70:call`;外部分支 `tizen_ci_shared.quickbuild_http:193:if` / raise `tizen_ci_shared.quickbuild_http:198:raise`;预期异常 `QuickBuildError`,**code `COOKIE_EXPIRED`** | |
| iframe 缺失 | `NO_GBS_REPORT`(:73-78);branch IDs `fetch_gbs_report:73:if` / `fetch_gbs_report:74:raise` | |
| iframe 页登录态 | **`EXTERNAL_BRANCH`**:本地调用点 `fetch_gbs_report:81:call`;外部分支 `tizen_ci_shared.quickbuild_http:193:if` / raise `tizen_ci_shared.quickbuild_http:198:raise`;预期异常 `QuickBuildError`,code `COOKIE_EXPIRED` | |
| parse 无 build-status 表 | `_looks_like_build_status_table` 未命中(:146);branch ID `parse_gbs_report_packages:146:if` | |
| 单包日志下载成功 | `download_gbs_package_buildlog`(:103 起);branch ID `download_gbs_package_buildlog:122:return` | |
| **iframe HTTP 非 200** | **`GBS_REPORT_DOWNLOAD_FAILED`(:82-86)**;branch IDs `fetch_gbs_report:82:if` / `fetch_gbs_report:83:raise`(v1.3 补:独立错误码,v1.2 漏) | |
| 单包日志:buildlog_url 缺失 | `GBS_PACKAGE_LOG_MISSING`(:111-115);branch IDs `download_gbs_package_buildlog:111:if` / `download_gbs_package_buildlog:112:raise`(v1.3 更正:**仅 URL 缺失**,无"构造失败") | |
| 单包日志:HTTP 非 200 | `GBS_PACKAGE_LOG_DOWNLOAD_FAILED`(:117-121);branch IDs `download_gbs_package_buildlog:117:if` / `download_gbs_package_buildlog:118:raise`(v1.3 更正:**非登录页**,该函数无登录检测) | |
...
| `render_report` **C:外层在、内层缺** | `selected_package` 存在但 `dest_file` 空;`source_fetch` 存在但 `change=None`、`error` 空 → 覆盖 **:62/:77/:85 的"假"侧**;branch IDs `render_report:62:if` / `render_report:77:if` / `render_report:85:if` | |
| `render_report` **D:中层在、末层缺** | `source_fetch` 与 `change` 均存在,但 `number=None`、`matching_patchset=None` → 覆盖 **:81/:83 的"假"侧**;branch IDs `render_report:81:if` / `render_report:83:if` | |
| `_primary_location` 态一:file+line | `f"{file}:{line}"`(:120-121);branch IDs `_primary_location:120:if` / `_primary_location:121:return` | |
| `_primary_location` 态二:仅 file(line 缺失) | `return file_value`(:122-123);branch IDs `_primary_location:122:if` / `_primary_location:123:return`(v1.4 补:v1.3 表中无) | |
| `_primary_location` 态三:无 file | `return "n/a"`(:124);branch ID `_primary_location:124:return` | |
| `_status_from_anchor` class 优先(failed / succeeded) | `class_names` 命中(:337/:339);branch IDs `_status_from_anchor:337:if` / `_status_from_anchor:338:return` / `_status_from_anchor:339:if` / `_status_from_anchor:340:return` | |
| `_status_from_anchor` text 回退(failed / succeeded) | 文本命中(:342-345);branch IDs `_status_from_anchor:342:if` / `_status_from_anchor:343:return` / `_status_from_anchor:344:if` / `_status_from_anchor:345:return` / `_status_from_anchor:346:return`(**只用 class 型 anchor 的用例永不走此路径**) | |
| **`_status_from_anchor` class 优先冲突样本**(v1.5 补,MAJOR) | class=`failed` + text=`Succeeded` → **断言结果为 `failed`**(:337 先于 :343);branch IDs `_status_from_anchor:337:if` / `_status_from_anchor:338:return`;**text-first 的错误实现在此必红**——仅"class 命中"样本无法证伪它 | |
| **`successful` 别名**(v1.5 补) | `succeeded`/`successful` 两写法均命中(:339 class 侧、:345 text 侧);branch IDs `_status_from_anchor:339:if` / `_status_from_anchor:340:return` / `_status_from_anchor:344:if` / `_status_from_anchor:345:return` | |
| `_looks_like_build_status_table` 三态 | branch IDs `_looks_like_build_status_table:296:if` / `_looks_like_build_status_table:297:return`;header fallback / 均不命中 `_looks_like_build_status_table:299:return`(v1.6 更正::287 是 def 行、不在 ID 全集,§11 的 ⊆ 检查会红) | |
| `_row_to_package` 成功态 href 有 / 无 | `buildlog_url` 由 `status_anchor.href` 决定;branch IDs `_row_to_package:325:ifexp` / `_row_to_package:326:return` | |
| `GbsReport.failed_packages` 过滤 | computed property(:51-53):含 failed 与非 failed 混合样本;branch IDs `GbsReport.failed_packages:53:compif` / `GbsReport.failed_packages:53:return` | |
| **arch 场景 A/B** | 见 §5 ⑧ 两场景;branch IDs `fetch_gbs_report:73:if` / `fetch_gbs_report:74:raise` / `fetch_gbs_report:94:return` | |
