# P4.9 skill-6 设计:tizen-triage-report 抽取 + gbs_report 关门(v1.8-FROZEN)
...
>
> **v1.6 修订(Codex 2 MAJOR;Claude Code freeze-ready + 1 MINOR + 3 NIT。
> **§11 selector 已验收成功**:三方按其字面实现独立实跑,首次得到**同一组
> 数**——gbs_report `decision 19 / terminal 24`、report.py `22 / 4`,
> 共 69 个 ID 零碰撞)**:①【MAJOR,Codex】**§5 表头两行未同步 C/D 裁决**
> ——仍写"A 真侧/B 全部假侧",与紧随其后的 v1.5 正文自相矛盾(补了 C/D
> 却没改表,**"定义节改了下游没跟"在本稿第三次**);按实测四组分工重写;
> ②【MAJOR,Codex】**两行登录态无本地 branch ID**——`_raise_if_login_page`
> 的 `raise` 在 shared `quickbuild_http:190`,本地(:70/:81)仅调用点,
> §11"每行须引用 ≥1 本地 ID"对其**不可满足**;新增具名 **`EXTERNAL_BRANCH`**
> 类别(绑定调用点 + 预期异常);③【MINOR,Claude Code】`:287` 是 def 行、
> 不在 ID 全集 → 改 `:296/:297/:299`;④【NIT,Codex】branch ID **唯一性
> fail-closed 断言**;⑤【NIT×3,Claude Code】模板级留白写死(裸 return /
> elif / assert·match 的计入口径);⑥【Codex】§8 DoD 补元规则一条;
> ⑦明确**不回填 §10**(Claude Code 表态:§10 判据是集合比对、不产生"声明
> 数值"这一步,回填反而给它造一个本不需要的手填值);§10 另采其"旧版要点集
> 由脚本从实质 diff 自动提取"的同源改进(建议,不阻塞)。
...
> **v1.7 修订(Codex 2 MAJOR + 1 MINOR;Claude Code freeze-ready + 2 MINOR)**:
> ①§5 表下"四个 fixture"旧句**逐字同步新表**(表已对、说明未跟——同一节内
> 矛盾,"定义改了下游没跟"第四次);②**selector 消除两套定义**——
> `Assert`/`match_case` 并入权威表,`match_case` 无 `lineno` 故 ID 用
> `case.pattern.lineno`(退回 `match.lineno`)、`kind=case`;补 **fail-closed:
> 未列入 selector 表的 AST 类型一律不计**;③**`EXTERNAL_BRANCH` 外部锚改准**
> `:190`(def 行)→ `:193:if`/`:198:raise`,钉 `COOKIE_EXPIRED`——**该错与本轮
> 变更③ 刚修掉的 `:287` 同类,在同一轮里重现**;④模板补第四类 `ast.BoolOp`;
> ⑤`:317` 补"空 `spec_name`"输入类。
> **v1.6 findings 状态**:§5 表下旧句=闭合;selector 两套=闭合;`:190` 锚=闭合;
> BoolOp 第四类=v1.7 补入、**v1.8 再修(见下)**;`COOKIE_EXPIRED`=闭合。
>
> **v1.8 修订(Codex 2 MAJOR)**:①【MAJOR】**BoolOp 条款不可机械执行且声明数
> 错**——§11 手写"14 处"**违反本节自订元规则**(数值须由 A₀ 产出),且沿本节
> HTMLParser 排除口径实测为 **7+1=8**(14 含被排除类中的 6 处);"操作数对应
> 不同输入类"又依赖**人工语义判断**、脚本不产 obligation/ID,只补 `:317` 而
> `:67/:116/:289/:291/:300/:339/:120` 未销账。**采 Codex 保守解**:删除泛化
> 条款、保留 `:317` 具体用例,并把"BoolOp 逐操作数义务"登记为**模板级待决项**;
> ②【MAJOR】**v1.7 整个修订块缺失**——违反 §9 自订硬纪律;本块即补录
> (v1.7 五项 + v1.6 findings 状态 + v1.8 两项)。
> **v1.7 findings 状态**:BoolOp=本轮改保守解;修订块=本轮补录。
...
|---|---|---|
| `DEFAULT_ARCHES` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `GbsReportPackage` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `GbsReport` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `fetch_gbs_report` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `download_gbs_package_buildlog` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `find_iframe_src` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `parse_gbs_report_packages` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_Anchor` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_Cell` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_Row` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_Table` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_CellBuilder` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_AnchorBuilder` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_IframeParser` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_ReportTableParser` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_looks_like_build_status_table` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_row_to_package` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_status_from_anchor` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_attrs_to_map` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_class_names` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `_normalize_text` | `tizen_triage_report/gbs_report.py` | `skill/tizen_triage_report` |
| `TriageReportData` | `tizen_triage_report/report.py` | `skill/tizen_triage_report` |
| `render_report` | `tizen_triage_report/report.py` | `skill/tizen_triage_report` |
| `_primary_location` | `tizen_triage_report/report.py` | `skill/tizen_triage_report` |
...
| 契约句 | 分支(代码锚) | 用例 |
|---|---|---|
| fetch 成功 | `fetch_gbs_report` 返回 `GbsReport`(:94);branch ID `fetch_gbs_report:94:return` | `test_gbs_report_fetch_uses_iframe_src_without_reencoding`; `test_fetch_gbs_report_preserves_raw_arch_in_url_and_packages` |
| 报表页登录态 | **`EXTERNAL_BRANCH`**:本地调用点 `fetch_gbs_report:70:call`;外部分支 `tizen_ci_shared.quickbuild_http:193:if` / raise `tizen_ci_shared.quickbuild_http:198:raise`;预期异常 `QuickBuildError`,**code `COOKIE_EXPIRED`** | `test_fetch_gbs_report_rejects_report_page_login` |
| iframe 缺失 | `NO_GBS_REPORT`(:73-78);branch IDs `fetch_gbs_report:73:if` / `fetch_gbs_report:74:raise` | `test_gbs_report_without_iframe_reports_no_gbs_report`; `test_fetch_gbs_report_no_iframe_error_preserves_raw_arch` |
| iframe 页登录态 | **`EXTERNAL_BRANCH`**:本地调用点 `fetch_gbs_report:81:call`;外部分支 `tizen_ci_shared.quickbuild_http:193:if` / raise `tizen_ci_shared.quickbuild_http:198:raise`;预期异常 `QuickBuildError`,code `COOKIE_EXPIRED` | `test_fetch_gbs_report_rejects_iframe_page_login` |
| parse 无 build-status 表 | `_looks_like_build_status_table` 未命中(:146);branch ID `parse_gbs_report_packages:146:if` | `test_parse_gbs_report_packages_ignores_non_status_tables` |
| 单包日志下载成功 | `download_gbs_package_buildlog`(:103 起);branch ID `download_gbs_package_buildlog:122:return` | `test_download_gbs_package_buildlog_returns_text` |
| **iframe HTTP 非 200** | **`GBS_REPORT_DOWNLOAD_FAILED`(:82-86)**;branch IDs `fetch_gbs_report:82:if` / `fetch_gbs_report:83:raise`(v1.3 补:独立错误码,v1.2 漏) | `test_gbs_report_iframe_download_failure_remains_retryable` |
| 单包日志:buildlog_url 缺失 | `GBS_PACKAGE_LOG_MISSING`(:111-115);branch IDs `download_gbs_package_buildlog:111:if` / `download_gbs_package_buildlog:112:raise`(v1.3 更正:**仅 URL 缺失**,无"构造失败") | `test_download_gbs_package_buildlog_requires_url` |
| 单包日志:HTTP 非 200 | `GBS_PACKAGE_LOG_DOWNLOAD_FAILED`(:117-121);branch IDs `download_gbs_package_buildlog:117:if` / `download_gbs_package_buildlog:118:raise`(v1.3 更正:**非登录页**,该函数无登录检测) | `test_download_gbs_package_buildlog_rejects_non_200` |
| **畸形行跳过 ×4**(v1.3 找回:v1.1 有"畸形行",v1.2 重写时被删) | `_row_to_package` 四个 `return None`:cells<3(:313)/ **空 `spec_name`**(BoolOp 左操作数)/ 表头行 `package name`(BoolOp 右操作数)——**两个不同输入类各需一例**(:317,v1.7 补左操作数;对照 :339 的 `successful` 别名已补,同一构造不得一处补一处漏)/ 无 status anchor(:320)/ status 解析失败(:323);branch IDs `_row_to_package:313:if` / `_row_to_package:314:return` / `_row_to_package:317:if` / `_row_to_package:318:return` / `_row_to_package:319:ifexp` / `_row_to_package:320:if` / `_row_to_package:321:return` / `_row_to_package:323:if` / `_row_to_package:324:return` | `test_row_to_package_rejects_short_row`; `test_row_to_package_rejects_empty_spec_name`; `test_row_to_package_rejects_header_row`; `test_row_to_package_rejects_missing_status_anchor`; `test_row_to_package_rejects_unknown_status` |
| `render_report` **A:全部可选字段齐备** | 覆盖**普通 13 条的"真"侧**(:45/:47/:49/:64/:66/:89/:91/:93/:100/:102/:104/:108/:111)**+ :62/:77/:85/:81/:83 的"真"侧**;并使 :53/:70 走 `else`(即其"假"侧);branch IDs `render_report:45:if` / `render_report:47:if` / `render_report:49:if` / `render_report:53:if` / `render_report:62:if` / `render_report:64:if` / `render_report:66:if` / `render_report:70:if` / `render_report:77:if` / `render_report:81:if` / `render_report:83:if` / `render_report:85:if` / `render_report:89:if` / `render_report:91:if` / `render_report:93:if` / `render_report:100:if` / `render_report:102:if` / `render_report:104:if` / `render_report:108:if` / `render_report:111:if` / `render_report:114:return` | `test_render_report_fixture_a_all_optional_fields_present` |
| `render_report` **B:全部可选字段缺失** | 覆盖**普通 13 条的"假"侧** **+ :53/:70 的"真"侧**(v1.6 更正:B **不**覆盖 :62/:77/:81/:83/:85——外层折叠后内层不可达);branch IDs `render_report:45:if` / `render_report:47:if` / `render_report:49:if` / `render_report:53:if` / `render_report:64:if` / `render_report:66:if` / `render_report:70:if` / `render_report:89:if` / `render_report:91:if` / `render_report:93:if` / `render_report:100:if` / `render_report:102:if` / `render_report:104:if` / `render_report:108:if` / `render_report:111:if` / `render_report:114:return` | `test_render_report_fixture_b_all_optional_fields_absent` |
| `render_report` **C:外层在、内层缺** | `selected_package` 存在但 `dest_file` 空;`source_fetch` 存在但 `change=None`、`error` 空 → 覆盖 **:62/:77/:85 的"假"侧**;branch IDs `render_report:62:if` / `render_report:77:if` / `render_report:85:if` | `test_render_report_fixture_c_outer_present_inner_absent` |
| `render_report` **D:中层在、末层缺** | `source_fetch` 与 `change` 均存在,但 `number=None`、`matching_patchset=None` → 覆盖 **:81/:83 的"假"侧**;branch IDs `render_report:81:if` / `render_report:83:if` | `test_render_report_fixture_d_middle_present_leaf_absent` |
| `_primary_location` 态一:file+line | `f"{file}:{line}"`(:120-121);branch IDs `_primary_location:120:if` / `_primary_location:121:return` | `test_primary_location_includes_file_and_line` |
| `_primary_location` 态二:仅 file(line 缺失) | `return file_value`(:122-123);branch IDs `_primary_location:122:if` / `_primary_location:123:return`(v1.4 补:v1.3 表中无) | `test_primary_location_returns_file_without_line` |
| `_primary_location` 态三:无 file | `return "n/a"`(:124);branch ID `_primary_location:124:return` | `test_primary_location_returns_na_without_file` |
| `_status_from_anchor` class 优先(failed / succeeded) | `class_names` 命中(:337/:339);branch IDs `_status_from_anchor:337:if` / `_status_from_anchor:338:return` / `_status_from_anchor:339:if` / `_status_from_anchor:340:return` | `test_status_from_anchor_prefers_status_classes` |
| `_status_from_anchor` text 回退(failed / succeeded) | 文本命中(:342-345);branch IDs `_status_from_anchor:342:if` / `_status_from_anchor:343:return` / `_status_from_anchor:344:if` / `_status_from_anchor:345:return` / `_status_from_anchor:346:return`(**只用 class 型 anchor 的用例永不走此路径**) | `test_status_from_anchor_falls_back_to_text` |
| **`_status_from_anchor` class 优先冲突样本**(v1.5 补,MAJOR) | class=`failed` + text=`Succeeded` → **断言结果为 `failed`**(:337 先于 :343);branch IDs `_status_from_anchor:337:if` / `_status_from_anchor:338:return`;**text-first 的错误实现在此必红**——仅"class 命中"样本无法证伪它 | `test_status_from_anchor_class_wins_over_conflicting_text` |
| **`successful` 别名**(v1.5 补) | `succeeded`/`successful` 两写法均命中(:339 class 侧、:345 text 侧);branch IDs `_status_from_anchor:339:if` / `_status_from_anchor:340:return` / `_status_from_anchor:344:if` / `_status_from_anchor:345:return` | `test_status_from_anchor_accepts_successful_alias` |
| `_looks_like_build_status_table` 三态 | branch IDs `_looks_like_build_status_table:296:if` / `_looks_like_build_status_table:297:return`;header fallback / 均不命中 `_looks_like_build_status_table:299:return`(v1.6 更正::287 是 def 行、不在 ID 全集,§11 的 ⊆ 检查会红) | `test_looks_like_build_status_table_covers_three_detection_states` |
| `_row_to_package` 成功态 href 有 / 无 | `buildlog_url` 由 `status_anchor.href` 决定;branch IDs `_row_to_package:325:ifexp` / `_row_to_package:326:return` | `test_row_to_package_builds_urls_with_and_without_href` |
| `GbsReport.failed_packages` 过滤 | computed property(:51-53):含 failed 与非 failed 混合样本;branch IDs `GbsReport.failed_packages:53:compif` / `GbsReport.failed_packages:53:return` | `test_gbs_report_failed_packages_filters_mixed_statuses` |
| **arch 场景 A/B** | 见 §5 ⑧ 两场景;branch IDs `fetch_gbs_report:73:if` / `fetch_gbs_report:74:raise` / `fetch_gbs_report:94:return` | `test_fetch_gbs_report_preserves_raw_arch_in_url_and_packages`; `test_fetch_gbs_report_no_iframe_error_preserves_raw_arch` |
...
**四个 fixture(v1.7 与表逐字同步;v1.6 表已改对而此处旧句未跟)**:
- **A 全字段齐备**:覆盖**普通 13 条 + `:62/:77/:85/:81/:83` 的"真"侧**,
  并使 `:53/:70` 走 `else`(即其**"假"侧**);
- **B 全字段缺失**:覆盖**普通 13 条的"假"侧 + `:53/:70` 的"真"侧**;
  **不覆盖** `:62/:77/:81/:83/:85`(外层折叠后内层**不可达**);
...
- [ ] **§11 准入(元规则)**:两模块 `decision_points`/`terminal_outcomes` 声明值
      **由 A₀ `branch_inventory.py` 实跑产出**(文档零手填数字);**ID 唯一性
      断言绿**;分支表所引 ID ⊆ 脚本全集,未引用 ID 已登记理由;
      **`EXTERNAL_BRANCH` 行绑定齐**(本地调用点/外部模块行号/预期异常);
- [ ] **P4.9 末批次清单核实**:本批**不新增实施 DEFERRED**(§3 关闭、§0.3
      归他批);§11 的 BoolOp 待决项仅属 skill 模板演进议题,不进入 P4.9
      末批次清单。
...
**与 §11 元规则的边界(v1.6 明确,Claude Code 表态)**:**元规则不回填 §10**
——§10 的判据是「旧版要点集 ⊆ 新版行集」的**集合比对**,全程不产生"声明
数值"这一步;强行回填会给 §10 制造一个它本不需要的手填值,**正是元规则要
防的东西**。
**§10 的同源改进(建议,不阻塞)**:"旧版要点集"亦应**由脚本从上一版实质
diff 自动提取**,而非人工列举——与 skill-4 §5.4.1-1「候选自动导出、人工
只许删」同型。这是"自动化输入面",与元规则的"自动化声明值"并列。
...
| 口径 | AST selector | 排除 |
|---|---|---|
| **decision_points** | `ast.If` ∪ `ast.IfExp` ∪ comprehension `ifs` ∪ **`ast.Assert`** ∪ **`ast.match_case`**(v1.7 并入,消除两套定义) | `_ReportTableParser` / `_IframeParser`(HTMLParser 回调,非契约面) |
| **terminal_outcomes** | `ast.Raise` ∪ `ast.Return` | 同上 |

**branch ID** = `qualname:lineno:kind`(如 `_row_to_package:318:return`),
分支表每行须引用 ≥1 个 branch ID。**ID 唯一性 fail-closed(v1.6,NIT)**:
脚本须断言 ID 全集**无重复**,重复即红(当前快照零碰撞;`kind` 字段正是
为消解 `GbsReport.failed_packages:53` 同行 `compif`+`return` 的碰撞)。

**`EXTERNAL_BRANCH` 类别(v1.6 新增,MAJOR)**:若某契约行的**分支与
`raise` 位于本批之外的模块**(实例:"报表页/iframe 页登录态"——
`_raise_if_login_page` 的**分支在 `tizen_ci_shared.quickbuild_http:193`、
raise 在 `:198`**(`:190` 是 def 行——v1.7 更正,该锚是本类别三项绑定中
**唯一可机械核验的一项**),本地 :70/:81 仅为调用点),则该行**不可能有诚实的本地 branch ID**。
此类行登记为 `EXTERNAL_BRANCH`,**必须绑定**:①本地调用点
`qualname:lineno:call`;②外部模块与行号;③**预期异常/返回**。
**其余行一律仍须本地 branch ID**——本类别不得用于规避本地枚举。

**模板级口径写死(v1.6,NIT×3;本批两模块均无影响,后续批次会撞)**:
①**裸 `return`**(无返回值)**计入** `terminal_outcomes`(本批的裸 return
全在被排除的 HTMLParser 回调内);②**`elif`** 在 AST 中是嵌套 `ast.If`、
**逐个计数**(本批无 elif);③**`assert`** 与 **`match`** 计入 `decision_points`——**已并入上方 selector
表**(v1.7:此前只写在模板口径里,与权威 selector 形成两套定义);
**`match` 按 `ast.match_case` 逐个计数**,因 `match_case` **无 `lineno`
属性**,其 ID 用 **`case.pattern.lineno`**(缺失时退回 `match.lineno`)
并以 `kind=case` 标注,形如 `qualname:<lineno>:case`;
④**`ast.BoolOp` 不计入 `decision_points`,且不设泛化义务(v1.8 保守解)**
——v1.7 曾立"各操作数若对应不同输入类须分行"的泛化条款,**两处失败**:
(a)其判定"是否不同输入类"**依赖人工语义**,脚本不产 obligation/ID,
与本节"机械可执行"相悖;(b)v1.7 同时手写"实存 14 处",**违反本节自订
元规则**(数值须由 A₀ 实跑产出),且沿本节 HTMLParser 排除口径实测应为
**7+1**(14 误含被排除类中的 6 处)。
**故:BoolOp 不产生任何自动义务**;`:317` 的"空 `spec_name`"作为**具体
用例保留在 §5 分支表**(它由人工审出、已入表,不依赖泛化规则)。
**登记为模板级待决项**:"BoolOp 逐操作数义务"若要成为门禁,须先解决
"输入类等价划分"的机械判定(如由 A₀ 生成 BoolOp obligation 集并要求
**全量**销账),**不在本批引入半机械条款**。该待决项只演进跨批次 skill
模板,不是 skill-6 的实施 DEFERRED,不进入 P4.9 末批次清单;
⑤**未列入本 selector 表的 AST 类型一律不计**(fail-closed,v1.7 补:
避免"未提及即由实现者裁量")。
