# P4.5 实现任务:campaign-repair-step 与联合对账(design v1.5.14-FROZEN)
...
1. **唯一契约权威** = `docs/clang-fix-campaign/design.md`
   (**v1.5.14-FROZEN**,冻结于 2026-08-04)。`design_changes/change_*.md`
   仅为历史;**任何与主文的冲突一律以主文为准**。
2. **停止并报告,永不自修设计**:实现中发现设计矛盾、缺失定义、
   无法实现的契约,立即**停止**,输出"矛盾报告"(引用 design.md
   行号 + 冲突描述 + 候选方案),等待裁决(将立案 change_39+)。
   禁止自行取舍、禁止"合理推断后继续"。
...
**P0-2 checker 四规则**:在
`docs/clang-fix-campaign/tools/check_design_doc.py` 新增:
- **CK-API-01**:提取文档内全部冻结 API 代码块,`ast.parse` 校验,
  语法错误即 FAIL;
- **CK-IDX-01**:design.md 与 `p45-implementation-prompt*.md` 中出现的
  `CREATE (UNIQUE )?INDEX` 索引名集合必须一致;
- **CK-XREF-01**:文内 `§n(.m)` 引用必须解析到存在的章节;
- **CK-MMD-01**:Mermaid 块节点先声明后引用。
每条规则配**至少一个失败 fixture**(mutation 样例,证明规则咬得住);
全部 fixture 与既有 20 个一起跑,输出形如 `self-test: 24/24`。
...
2. `campaign_state` 冻结 API 族(签名逐字对齐 design.md §4.2):
   ...
   `find_unlinked_pass`(v1.5.14 形态:**无 round_index 形参**)、
